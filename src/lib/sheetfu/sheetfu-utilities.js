...
function getItemValuesById(
  ...
  var ids = sheet
    .getRange(headerRow + 1, idColumnIndex, numRows - headerRow, 1)
    .getValues();

  var rowIndex = isSorted
    ? ids.binaryIndexOf(lookupValue)
    : ids.indexOf2d(lookupValue);
  if (rowIndex === -1) {
    return undefined;
  }
  var itemRow = sheet
    .getRange(rowIndex + headerRow + 1, 1, 1, numColumns)
    .getValues();
  var item = {};
