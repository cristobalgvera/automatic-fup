...
const baseClass_ = FirebaseApp_.Base.prototype;
 ...
 * @returns {object} the auth token granting access to firebase
 */
baseClass_.createAuthToken = function (
  userEmail,
  optAuthData,
  serviceAccountEmail,
  privateKey
 ...
 * @returns {object} the auth token granting access to firebase
 */
baseClass_.createAuthTokenFromServiceAccount_ = function (
  userEmail,
  optCustomClaims
) {
  ...
  optCustomClaims &&
    Object.keys(optCustomClaims).forEach(item => {
  ...
  let stringifiedBody = JSON.stringify(body); // Stringified after adding optional auth data
  stringifiedBody = Utilities.base64Encode(stringifiedBody);
  const signature = Utilities.computeRsaSha256Signature(
    header + '.' + stringifiedBody,
    this.base.privateKey
  );
  return (
    header + '.' + stringifiedBody + '.' + Utilities.base64Encode(signature)
  );
};
...
baseClass_.createLegacyAuthToken_ = function (userEmail, optCustomClaims) {
  ...
  let stringifiedPayload = JSON.stringify(payload); // Stringified after adding optional auth data
  stringifiedPayload = Utilities.base64EncodeWebSafe(stringifiedPayload);
  const hmac = Utilities.computeHmacSha256Signature(
    header + '.' + stringifiedPayload,
    this.base.secret
  );
  return (
    header +
    ...
    stringifiedPayload +
    '.' +
    Utilities.base64EncodeWebSafe(hmac)
  );
};
...
baseClass_.getData = function (path, optQueryParameters) {
  ...
  const [res] = FirebaseApp_._buildAllRequests(
    [
      {
        method: 'get',
        path: path,
...
baseClass_.getUrlFromPath = function (path) {
  ...
  const keysInPath = path.split('/');
  for (let i = 0; i < keysInPath.length; i++) {
    url +=
      encodeURIComponent(FirebaseApp.encodeAsFirebaseKey(keysInPath[i])) + '/';
  }
  return url;
};
...
FirebaseApp_.NORETRY_ERRORS[
  FirebaseApp_.NORMALIZED_ERRORS.INVALID_DATA_BIS
] = true;
FirebaseApp_.NORETRY_ERRORS[
  FirebaseApp_.NORMALIZED_ERRORS.URLFETCHAPP_CRASH
] = true;
...
FirebaseApp_._buildAllRequests = function (requests, db) {
  let authToken = db.base.secret,
    finalRequests = [],
    headers = {};

  // Deep copy of object to avoid changing it
  /** @type {Array.<FirebaseApp_.request>} */
  const initialRequests = JSON.parse(JSON.stringify(requests));
  ...
  for (let i = 0; i < initialRequests.length; i++) {
    ...
    if (typeof initialRequests[i] === 'string') {
      ...
      initialRequests[i].optQueryParameters =
        initialRequests[i].optQueryParameters || {};
    ...
    // Add data if any
    'data' in initialRequests[i] &&
      (requestParam.payload = JSON.stringify(initialRequests[i].data));

    // Add Authorization header if necessary
    headers['Authorization'] &&
      (requestParam.headers['Authorization'] = headers['Authorization']);
    ...
    const parameters = [];
    for (const key in initialRequests[i].optQueryParameters) {
      // Encode non boolean parameters (except whitelisted keys)
      if (
        !FirebaseApp_._keyWhiteList[key] &&
        typeof initialRequests[i].optQueryParameters[key] === 'string'
      ) {
        initialRequests[i].optQueryParameters[key] = encodeURIComponent(
          '"' + initialRequests[i].optQueryParameters[key] + '"'
        );
      }
    ...
    requestParam.url =
      db.base.url +
      initialRequests[i].path.replace(/%/g, '%25').replace(/\+/g, '%2b') +
      '.json' +
  ...
  for (let j = 0; j < initialRequests.length; j++) {
    data.push(
...
FirebaseApp_._sendAllRequests = function (
  ...
  let responses;
  let failureOnUrlFetchApp = false;
  ...
  if (finalRequests.length === 1) {
    ...
    if (typeof ErrorHandler !== 'undefined') {
      responses = [
        ...
        // best to throw error
        ErrorHandler.urlFetchWithExpBackOff(
          finalRequests[0].url,
          finalRequests[0],
          {
            throwOnFailure: true,
    ...
    if (failureOnUrlFetchApp) {
      ...
      if (FirebaseApp_._methodWhiteList[finalRequests[0].method]) {
        ...
        responses = [
          new FirebaseApp_.FetchResponse(
            0,
            '{"error":"' +
              FirebaseApp_.NORMALIZED_ERRORS.URLFETCHAPP_CRASH +
              '"}'
  ...
  for (let i = 0; i < responses.length; i++) {
    const responseCode = responses[i].getResponseCode();
    ...
    let responseContent = responses[i].getContentText();
    ...
    try {
      if (
        typeof HANDLE_FORBIDDEN_CHARS_IN_FIREBASE_KEYS !== 'undefined' &&
        HANDLE_FORBIDDEN_CHARS_IN_FIREBASE_KEYS == true
      ) {
        ...
        var keysToReplace = {};
        responseParsed = JSON.parse(responseContent, (key, value) => {
          // Alter the behavior of the parsing process to find keys to decode
          const encodedKey = FirebaseApp.decodeFirebaseKey(key);
          if (encodedKey != key) {
        ...
        if (Object.keys(keysToReplace).length) {
          for (const encodedKey in keysToReplace) {
            // make sure we only replace Firebase Keys, not values
            const encodedKeyEscaped = encodedKey.replace(
              /[.*+?^${}()|[\]\\]/g,
              '\\$&'
            ); // $& means the whole matched string
            const encodedKeyRegExp = new RegExp(
              '"' + encodedKeyEscaped + '":',
              'g'
            );
            responseContent = responseContent.replace(
              encodedKeyRegExp,
              '"' + keysToReplace[encodedKey] + '":'
            );
          }
    ...
    if (
      ...
      if (!originalsRequests[i].path) {
        ...
        for (const pathAsKey in originalsRequests[i].data) {
          originalsRequests[i].data[pathAsKey] = FirebaseApp_._encodeAllKeys(
            originalsRequests[i].data[pathAsKey]
          );
        }
      } else {
        originalsRequests[i].data = FirebaseApp_._encodeAllKeys(
          originalsRequests[i].data
        );
      }
  ...
  if (
    ...
    n <= 6 &&
    (n > 0 ||
      originalsRequests.length == 1 ||
      (errorCount <= 100 && errorCount < originalsRequests.length / 4))
  ) {
...
FirebaseApp_._encodeAllKeys = function (object) {
  const keysToReplace = {};
  object = JSON.stringify(object, (key, value) => {
    // Alter the behavior of the stringification process to find keys to encode
    const encodedKey = FirebaseApp.encodeAsFirebaseKey(key);
    if (encodedKey != key) {
      keysToReplace[key] = encodedKey;
    }
    return value;
  });
  for (const invalidKey in keysToReplace) {
    // make sure we only replace keys, not values
    const invalidKeyEscaped = invalidKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // $& means the whole matched string
    const invalidKeyRegExp = new RegExp('"' + invalidKeyEscaped + '":', 'g');
    object = object.replace(
      invalidKeyRegExp,
      '"' + keysToReplace[invalidKey] + '":'
    );
  }
